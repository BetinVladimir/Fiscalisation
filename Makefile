.PHONY: deps test test-race postgres-integration typecheck contract-test security-test web-build native-bundle android-build ios-build native-regression smart-device-test iot-test compose-check compose-e2e regression full-regression
deps:
...
compose-check:
	FISCAL_DB_PASSWORD=test WEBHOOK_SIGNING_KEY=dev-key docker compose -f compose.fiscalisation.yaml -f compose.fiscalisation.dev.yaml config >/dev/null
	FISCAL_DB_PASSWORD=test WEBHOOK_SIGNING_KEY=prod-key OIDC_ISSUER=https://id.example.test OIDC_AUDIENCE=beefiscal OIDC_JWKS_URL=https://id.example.test/jwks BLE_SIGNING_KEY=production-ble-key-at-least-32-bytes FISCAL_SITE=https://fiscal.example.test docker compose -f compose.fiscalisation.yaml -f compose.fiscalisation.prod.yaml config >/dev/null
	MINIPOS_DB_PASSWORD=test FISCAL_PUBLIC_BASE_URL=http://fiscal.test/public/v1 WEBHOOK_VERIFICATION_KEY=dev-key docker compose -f compose.minipos.yaml -f compose.minipos.dev.yaml config >/dev/null
	MINIPOS_DB_PASSWORD=test FISCAL_PUBLIC_BASE_URL=https://fiscal.example.test/public/v1 FISCAL_OAUTH_TOKEN_URL=https://id.example.test/token FISCAL_OAUTH_CLIENT_ID=minipos FISCAL_OAUTH_CLIENT_SECRET=production-client-secret OIDC_ISSUER=https://id.example.test OIDC_AUDIENCE=beeminipos OIDC_JWKS_URL=https://id.example.test/jwks WEBHOOK_VERIFICATION_KEY=prod-key MINIPOS_SITE=https://pos.example.test docker compose -f compose.minipos.yaml -f compose.minipos.prod.yaml config >/dev/null
compose-e2e:
	./scripts/e2e-two-compose.sh
regression: test-race typecheck contract-test security-test evidence-test web-build native-bundle smart-device-test iot-test compose-check
