...
function context(v:Ticket){return `${v.TenantID}|${v.RegisterID}|${v.DeviceID}|${v.SessionID}|${BLE_PROTOCOL_VERSION}`}
...
export class BleClientHandshake{
 ...
 async challenge(raw:Uint8Array):Promise<Uint8Array>{
  ...
  const shared=x25519Shared(this.pair.privateKey,edgePublic),ticketHash=hash256(te.encode(this.session.signed_session_ticket)),salt=hash256(concat(ticketHash,this.clientNonce,edgeNonce)),secret=hkdf256(shared,salt,te.encode("BeeFiscal BLE v1|"+context(parseTicket(this.session.signed_session_ticket))));
