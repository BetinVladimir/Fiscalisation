...
class NativeBleBootstrap implements NativeBleBootstrapContract{
 ...
 async sendComplianceIntentAndWait(intent:unknown,messageId:string,attMtu=185,timeoutMs=30_000):Promise<BleFiscalResult>{if(!this.handshake.canSendComplianceIntent)throw new Error("BLE_NOT_READY");const result=this.results.wait(messageId,timeoutMs);try{const chunks=chunkPlaintext(encodeCanonical(intent),attMtu);for(let i=0;i<chunks.length;i++)await this.write(this.session!.command_characteristic_uuid,await this.handshake.frameSession().seal(messageId,i,chunks.length,0,chunks[i]))}catch(e){this.results.reject(messageId,e)}return result}
