...
export type ActiveBleBinding={tenant_id:string;location_id:string;register_id:string;edge_id:string;device_id:string;binding_version:number;expires_at:string};
export type RestCheckout=()=>Promise<{state:string;operation_id?:string;fiscal_reference?:string|null}>;
export async function probeFinalDevice(operationId:string,binding:ActiveBleBinding,ble:Pick<WebBleBootstrap,"sendFiscalCommandAndWait">){const envelope=await buildDeviceProbeEnvelope(operationId,{tenantId:binding.tenant_id,locationId:binding.location_id,registerId:binding.register_id,deviceId:binding.device_id,fencingToken:binding.binding_version});const result=await ble.sendFiscalCommandAndWait(envelope,operationId,185,10_000);return result.state==="READY"}
