...
export class WebBleBootstrap{
 ...
 get canSendComplianceIntent(){return this.handshake.canSendComplianceIntent}
 ...
 async connectFromUserGesture(session:BleSessionPackage,onEvent:(raw:Uint8Array)=>void){
  ...
  const hello=await this.handshake.start(session);
  const advertisingIdentity=authorizedAdvertisingIdentity(session.advertising_identity,session.edge_id);
  this.device=await (navigator as any).bluetooth.requestDevice({filters:[{services:[session.service_uuid]}]}) as Device;
  if(!matchesAdvertisingIdentity(this.device,advertisingIdentity)){this.device.gatt?.disconnect?.();this.device=undefined;throw new Error("BLE_ADVERTISING_IDENTITY_MISMATCH")}
  ...
  await this.write(this.control,hello);return {state:"HELLO_SENT" as const,ready};
 }
 async sendComplianceIntent(intent:unknown,messageId:string,attMtu=185){if(!this.handshake.canSendComplianceIntent)throw new Error("BLE_NOT_READY");const payload=encodeCanonical(intent),chunks=chunkPlaintext(payload,attMtu);for(let i=0;i<chunks.length;i++)await this.write(this.command,await this.handshake.frameSession().seal(messageId,i,chunks.length,0,chunks[i]));return {messageId,chunks:chunks.length}}
 async sendComplianceIntentAndWait(intent:unknown,messageId:string,attMtu=185,timeoutMs=30_000):Promise<BleFiscalResult>{const result=this.results.wait(messageId,timeoutMs);try{await this.sendComplianceIntent(intent,messageId,attMtu)}catch(e){this.results.reject(messageId,e)}return result}
 private async handleHandshakeEvent(raw:Uint8Array,onEvent:(raw:Uint8Array)=>void,resolve:()=>void,reject:(e:unknown)=>void){try{if(this.handshake.currentState==="HELLO_SENT"){const v=decodeStrict<ControlMessage>(raw);if(v.type!=="CHALLENGE")throw new Error("BLE_CHALLENGE_EXPECTED");await this.write(this.control,await this.handshake.challenge(raw));return}if(this.handshake.currentState==="AUTH_SENT"){this.handshake.ready(raw);resolve();return}if(this.handshake.canSendComplianceIntent){const frame=await this.handshake.frameSession().open(raw);let flow:ReturnType<BleMessageAssembler["accept"]>;try{flow=this.assembler.accept(frame)}catch(e){const reason=e instanceof Error&&e.message==="BLE_BUSY"?"BUSY":"BAD_CHUNK";await this.sendFlowNack(frame.messageId,frame.counter,reason);throw e}await this.sendFlowAck(frame.messageId,frame.counter,flow);if(flow.complete){this.results.accept(frame.messageId,flow.complete);onEvent(flow.complete)}}}catch(e){reject(e)}}
