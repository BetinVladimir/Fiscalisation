...
import { MVP1_DEVICE_PROFILES, profileById, validateCompositeProfile } from "./deviceProfiles.ts";

test("MVP1 compatibility matrix exposes the three exact physical profiles", () => {
  assert.deepEqual(MVP1_DEVICE_PROFILES.map(x => x.id), ["DATECS_BLUECASH50_EMBEDDED", "DATECS_DP150_BLUEPAD50", "DAISY_COMPACT_S01"]);
