...
func TestTenantShiftCloseRequiresFinalZReport(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests++
		if r.URL.Path != "/registers/register-1/reports" || r.Method != http.MethodPost {
	...
	shift, err := s.OpenShiftForTenant("register-1", employee.ID, "tenant-1")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAmbiguousZReportBlocksShiftAndNextShift(t *testing.T) {
	...
	if _, err = s.OpenShiftForTenant("register-1", employee.ID, "tenant-1"); err == nil {
...
func TestBlockedZCloseReconcilesByReadingOriginalOperationOnly(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		switch {
		case r.Method == http.MethodPost && r.URL.Path == "/registers/register-1/reports":
			postRequests++
