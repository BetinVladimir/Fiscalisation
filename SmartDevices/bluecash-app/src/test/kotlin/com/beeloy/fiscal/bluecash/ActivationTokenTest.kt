...
class ActivationTokenTest {
    private fun token(payload: String) = "e30.${Base64.getUrlEncoder().withoutPadding().encodeToString(payload.toByteArray())}.signature"

    @Test fun bindsOrganizationLocationAndDevice() {
        val jwt = token("""{"aud":"beefiscal-bluecash-activation","organization_id":"org-1","location_id":"loc-1","device_id":"dev-1","app_instance_id":"app-1","exp":2000000000}""")
        val claims = ActivationToken.parseAndBind(jwt, "org-1", "loc-1", "dev-1", Instant.ofEpochSecond(1900000000))
    ...
    fun rejectsForeignLocation() {
        val jwt = token("""{"aud":"beefiscal-bluecash-activation","organization_id":"org-1","location_id":"loc-2","device_id":"dev-1","app_instance_id":"app-1","exp":2000000000}""")
        ActivationToken.parseAndBind(jwt, "org-1", "loc-1", "dev-1", Instant.ofEpochSecond(1900000000))
    }

    @Test fun loginIsRequiredBeforeToken() {
...
class TokenChunkAssemblerTest {
    @Test fun reassemblesOrderedFramesAndRejectsConflicts() {
