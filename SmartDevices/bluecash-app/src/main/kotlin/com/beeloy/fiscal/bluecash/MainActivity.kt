...
class MainActivity : Activity() {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    processor =
      BlueCashCommandProcessor(
        fiscalPort,
        DatecsAndroidPaymentPort(this, BoricaPinpadCodec(store = journal)),
        journal
      )
    ...
    content.addView(
      TextView(this).apply {
        text =
          "Устройството създава собствен неекспортируем ключ. Организацията не се въвежда тук — тя се определя от профила на администратора в BeeFiscalApp."
      }
    )
    ...
    content.addView(qr, ViewGroup.LayoutParams(640, 640))
    content.addView(
      Button(this).apply {
        text = "Създай заявка за активация"
        setOnClickListener { beginActivation() }
      }
    )
    content.addView(
      Button(this).apply {
        text = "Провери потвърждението"
        setOnClickListener { pollCredential() }
      }
    )
    content.addView(
      TextView(this).apply {
        text =
          "Device instance: ${identity.deviceInstanceId}\nBackend URL се инжектира от подписаната DEV/PROD build pipeline. MQTT/BLE операции не стартират преди ACTIVE acknowledgement."
      }
    )
    setContentView(
  ...
  private fun field(parent: LinearLayout, hint: String, initial: String = ""): EditText =
    EditText(this).also {
      it.hint = hint
      it.setText(initial)
      it.minHeight = 56
  ...
  private fun client(): DeviceBootstrapClient {
    check(BuildConfig.FISCAL_BACKEND_URL.startsWith("https://")) {
      "SIGNED_BUILD_BACKEND_NOT_CONFIGURED"
    }
    return DeviceBootstrapClient(BuildConfig.FISCAL_BACKEND_URL, identity, secrets)
  }
  private fun beginActivation() =
    background("CREATING_REQUEST") {
      require(serial.text.toString().isNotBlank() && fmin.text.toString().isNotBlank()) {
        "SERIAL_AND_FMIN_REQUIRED"
      }
      val pending =
        client()
          .begin(
            "DATECS",
            "BLUECASH_50",
            serial.text.toString().trim(),
            fmin.text.toString().trim(),
            firmware.text.toString().trim(),
            listOf("FISCAL_DEVICE", "PAYMENT_TERMINAL")
          )
      val payload =
        "${pending.verificationUri}?user_code=${pending.userCode}&activation_request_id=${pending.requestId}"
      runOnUiThread {
        code.text = pending.userCode
        qr.setImageBitmap(qrBitmap(payload, 640))
        status.text = "Състояние: WAITING_FOR_ADMIN (${pending.expiresAt})"
      }
    }
  private fun pollCredential() =
    background("CHECKING_CONFIRMATION") {
      val binding = client().pollCredential()
      activationMqtt =
        DeviceActivationMqtt(identity, secrets, binding) { active -> startManagedRuntime(active) }
          .also { it.start() }
      runOnUiThread {
        status.text =
          "Състояние: CREDENTIAL_ISSUED; WAITING_FOR_SIGNED_ACTIVE_ACK\nOrganization: ${binding.organizationId}\nLocation: ${binding.locationId}\nRegister: ${binding.registerId}"
      }
    }
  private fun startManagedRuntime(binding: ActivatedBinding) {
    ...
    val password = operatorPassword.text.toString()
    require(password.isNotBlank()) { "OPERATOR_PASSWORD_REQUIRED" }
    ...
    operationalMqtt =
      BlueCashMqttRuntime(
          BlueCashMqttConfig(
            brokerUri = binding.mqttTlsUri,
            clientId = "beefiscal-${binding.deviceId}",
            tenantId = binding.organizationId,
            deviceId = binding.deviceId,
            registerId = binding.registerId,
            bindingGeneration = binding.bindingVersion,
            commandHmacKey = decoder.decode(binding.commandHmacKey),
            syncAckHmacKey = decoder.decode(binding.syncAckHmacKey),
            socketFactory = DeviceTLS.socketFactory(binding),
            fiscalOperatorNumber = op,
            fiscalOperatorPassword = password,
            tillNumber = till
          ),
          processor,
          journal,
        ...
        .also { it.start() }
    transactionGatt =
      BlueCashTransactionGattServer(
          this,
          decoder.decode(binding.bleTicketHmacKey),
          BlueCashBleBinding(
    ...
    if (!issuer.isNullOrBlank() && !kid.isNullOrBlank() && !publicKey.isNullOrBlank()) {
      ...
      deploymentManager =
        SpaDeploymentManager(
            this,
            binding.spaDeploymentDescriptorURL,
            binding.spaDeploymentSigningKID,
            binding.spaDeploymentPublicKeyDERBase64
          )
          .also { manager ->
            deploymentScheduler.scheduleWithFixedDelay(
              { manager.checkAndActivate() },
              0,
              6,
              TimeUnit.HOURS
            )
      ...
      localHttp =
        BlueCashLocalHttpServer(
            ...
            executor::execute,
            { operationResult(it) },
            processor::fiscalReachable,
            processor::paymentReachable,
            manager::activeRoot,
            manager::state
          )
  ...
  private fun background(state: String, work: () -> Unit) {
    status.text = "Състояние: $state"
    Thread {
        runCatching(work).onFailure { error ->
          runOnUiThread { status.text = "Отказ: ${error.message}" }
        }
      }
      .start()
  }
  private fun qrBitmap(value: String, size: Int): Bitmap {
    ...
    return Bitmap.createBitmap(size, size, Bitmap.Config.RGB_565).also { image ->
      for (x in 0 until size) for (y in 0 until size) image.setPixel(
        x,
        y,
  ...
  private fun operationResult(id: String): Map<String, Any?>? {
    val row =
      journal.find(id).lastOrNull {
        it.type in
          setOf(
            "FISCALIZED",
            "REVERSED",
    ...
    values.putIfAbsent("state", row.type)
    values["operation_id"] = id
    return values
  }
  override fun onDestroy() {
    deploymentScheduler.shutdownNow()
    localHttp?.stop()
    transactionGatt?.stop()
    operationalMqtt?.stop()
    activationMqtt?.stop()
    fiscalPort.close()
    journal.close()
    super.onDestroy()
