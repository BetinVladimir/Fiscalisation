...
class LocalFiscalTokenVerifier(
    ...
    private val key = KeyFactory.getInstance("EC").generatePublic(
    ...
    fun verify(raw: String, requiredScope: String): LocalFiscalAuthority {
        ...
        require(header.getString("alg") == "ES256" && header.getString("kid") == keyId) { "TOKEN_HEADER" }
        ...
        require(
            body.getString("iss") == issuer &&
                "beeloy-local-fiscal-adapter" in audiences && adapterId in audiences &&
                body.getLong("nbf") <= now+30 && body.getLong("exp") > now-30 && body.getLong("exp")-issuedAt<=930,
        ...
        return LocalFiscalAuthority(
            body.getString("sub"), tenantId, locationId, registerId,
            body.getString("operator_id"), body.getString("shift_id"), adapterId,
            generation, scopes, Instant.ofEpochSecond(body.getLong("exp")),body.getString("jti"),
        )
    }

    private fun audiences(value: JSONObject): Set<String> = when (val raw = value.get("aud")) {
        is String -> setOf(raw)
    ...
    private fun p1363ToDer(p1363: ByteArray): ByteArray {
        fun positiveInteger(raw: ByteArray): ByteArray {
            ...
            val stripped = if (firstNonZero < 0) byteArrayOf(0) else raw.copyOfRange(firstNonZero, raw.size)
