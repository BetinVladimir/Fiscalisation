...
object ActivationToken {
    ...
    private fun number(json: String, key: String): Long? = Regex("\\\"${Regex.escape(key)}\\\"\\s*:\\s*([0-9]+)").find(json)?.groupValues?.get(1)?.toLongOrNull()

    fun parseAndBind(jwt: String, expectedOrganization: String, expectedLocation: String, expectedDevice: String, now: Instant = Instant.now()): ActivationClaims {
        val parts = jwt.split('.')
