...
/**
 * Trusted-LAN adapter for the cached Web POS. It authenticates local tokens and forwards the same
 * canonical command envelope used by BLE and MQTT; it does not implement a separate fiscal path.
 */
class BlueCashLocalHttpServer(
  ...
  override fun serve(session: IHTTPSession): Response =
    runCatching { route(session) }
      .getOrElse {
        problem(
          if (it is LocalHttpFailure) it.status
          else if (it is IllegalArgumentException) 422 else 500,
          it.message ?: "LOCAL_HTTP_FAILURE"
        )
      }
  ...
  private fun route(session: IHTTPSession): Response {
    ...
    if (session.method == Method.GET && path == "/deployment") {
      ...
      return json(
        ...
        mapOf(
          ...
          "build_id" to value.buildId,
          "state" to value.state,
          "error_code" to value.errorCode
        )
      )
    }
    ...
    val authority =
      authorize(
        session,
        if (path.contains("printer")) "fiscal.printer_test"
        else if (session.method == Method.GET) "fiscal.read" else "fiscal.execute"
      )
    if (session.method == Method.GET && (path == "/readyz" || path == "/device")) {
      val fiscal = fiscalReady()
      val payment = paymentReady()
      return json(
        if (fiscal) 200 else 503,
        mapOf(
    ...
    if (session.method == Method.POST && path == "/intents") {
      ...
      val canonicalPayload =
        rawIntent.optJSONObject("canonical_payload") ?: error("CANONICAL_PAYLOAD_REQUIRED")
      val digest =
        MessageDigest.getInstance("SHA-256")
          .digest(CanonicalJson.encode(canonicalPayload).toByteArray())
          .joinToString("") { "%02x".format(it) }
      require(digest == rawIntent.optString("payload_sha256")) { "PAYLOAD_DIGEST_MISMATCH" }
      val intent = jsonObjectToMap(rawIntent)
      val id =
        intent["client_operation_id"] as? String
          ?: intent["intent_id"] as? String ?: error("OPERATION_ID_REQUIRED")
      require(session.headers["idempotency-key"] == id) { "IDEMPOTENCY_KEY_MISMATCH" }
      require(
        intent["tenant_id"] == authority.tenantId &&
          intent["location_id"] == authority.locationId &&
          intent["register_id"] == authority.registerId &&
          intent["edge_device_id"] == authority.adapterDeviceId &&
          (intent["binding_generation"] as? Number)?.toLong() == authority.bindingGeneration
      ) {
        "INTENT_BINDING_FENCE"
      }
      require(
        (intent["intent_id"]
          ?: id) == id && (intent["action"] ?: intent["command"]) == intent["command"]
      ) {
      ...
      val normalized =
        if (intent.containsKey("action")) intent
        else intent + mapOf("intent_id" to id, "action" to intent["command"])
  ...
  private fun authorize(session: IHTTPSession, scope: String): LocalFiscalAuthority {
    val raw =
      session.headers["authorization"]?.takeIf { it.startsWith("Bearer ") }?.removePrefix("Bearer ")
        ?: throw LocalHttpFailure(401, "TOKEN_REQUIRED")
    return runCatching { verifier.verify(raw, scope) }
      .getOrElse { throw LocalHttpFailure(401, it.message ?: "TOKEN_INVALID") }
  }
  ...
  private fun static(session: IHTTPSession, path: String): Response {
    ...
    return newFixedLengthResponse(
      ...
      .apply {
        ...
        addHeader(
          "Cache-Control",
          if (relative == "index.html") "no-cache" else "public,max-age=31536000,immutable"
        )
      }
  }
