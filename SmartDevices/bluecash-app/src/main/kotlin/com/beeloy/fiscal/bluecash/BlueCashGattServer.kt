...
/**
 * BLE provisioning endpoint. Chunk assembly is bounded and the activation token is applied once.
 */
class BlueCashGattServer(
  private val context: Context,
  private val controller: BlueCashActivationController
) {
  companion object {
  ...
  private val callback =
    object : BluetoothGattServerCallback() {
      ...
      override fun onCharacteristicWriteRequest(
        ...
        val result =
          if (characteristic.uuid == TOKEN_UUID && offset == 0 && !preparedWrite)
            runCatching {
                val token = chunks.accept(device.address, value.toString(Charsets.UTF_8))
                status =
                  if (token == null) "RECEIVING_TOKEN"
                  else {
                    controller.acceptToken(token)
                    "ACTIVATED"
                  }
              }
              .fold(
                ...
                {
                  status = it.message ?: "ACTIVATION_REJECTED"
      ...
      override fun onCharacteristicReadRequest(
        ...
        server?.sendResponse(
          ...
          offset,
          if (valid) bytes.copyOfRange(offset, bytes.size) else null
        )
      }
    }
...
/** Reassembles ordered chunks and rejects gaps, conflicting duplicates and oversized tokens. */
class TokenChunkAssembler {
  ...
  @Synchronized
  fun accept(peer: String, frame: String): String? {
    ...
    require(
      transferId.matches(Regex("[A-Za-z0-9-]{8,64}")) &&
        total in 1..32 &&
        index in 1..total &&
        fields[4].isNotEmpty()
    ) {
      "ACTIVATION_FRAME_INVALID"
    }
    ...
    val token =
      (1..total).joinToString("") { transfer.parts[it] ?: error("ACTIVATION_FRAME_INCOMPLETE") }
