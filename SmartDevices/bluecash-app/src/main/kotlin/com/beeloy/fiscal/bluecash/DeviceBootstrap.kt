...
data class ActivatedBinding(
  ...
  val mqttTlsUri: String,
  val mqttWssUri: String?,
  val deviceId: String,
  ...
  val commandHmacKey: String,
  val syncAckHmacKey: String,
  val bleTicketHmacKey: String,
  val unpPrefix: String,
  val unpRangeStart: Long,
  val unpRangeEnd: Long,
  val localTokenIssuer: String? = null,
  val localTokenSigningKID: String? = null,
  val localTokenPublicKeyDERBase64: String? = null,
  val spaDeploymentDescriptorURL: String? = null,
  val spaDeploymentSigningKID: String? = null,
  val spaDeploymentPublicKeyDERBase64: String? = null
)

/**
 * Owns the non-exportable signing identity used for bootstrap, MQTT mTLS and journal signatures.
 */
/** Owns the non-exportable Android Keystore key used for device identity and journal signatures. */
class AndroidDeviceIdentity(private val context: Context) : TransactionSigner {
  companion object {
    const val KEY_ALIAS = "beefiscal.device.identity.v1"
    private const val PREFS = "beefiscal_device_identity"
  }
  private val store = KeyStore.getInstance("AndroidKeyStore").apply { load(null) }
  val deviceInstanceId: String by lazy {
    val preferences = context.getSharedPreferences(PREFS, Context.MODE_PRIVATE)
    preferences.getString("device_instance_id", null)
      ?: UUID.randomUUID().toString().also {
        preferences.edit().putString("device_instance_id", it).apply()
      }
  }
        ...
        .encodeToString(
          value.toByteArray().let { bytes ->
            when {
              ...
              else -> error("INVALID_P256_COORDINATE")
            }
          }
        )
    return JSONObject()
      .put("kty", "EC")
      .put("crv", "P-256")
      .put("x", coordinate(key.w.affineX))
      .put("y", coordinate(key.w.affineY))
  }

  fun sign(raw: String): String =
    Signature.getInstance("SHA256withECDSA").run {
      initSign(store.getKey(KEY_ALIAS, null) as java.security.PrivateKey)
  ...
  override val keyId: String
    get() {
      val digest =
        MessageDigest.getInstance("SHA-256").digest(CanonicalJson.encode(publicJwk()).toByteArray())
...
/** Stores only backend-issued credentials; the private identity key never enters this store. */
class DeviceCredentialStore(private val context: Context) {
  companion object {
    private const val ALIAS = "beefiscal.bootstrap.storage.v1"
    private const val PREFS = "beefiscal_bootstrap"
  }
  ...
  fun put(name: String, value: String) {
    ...
    val encoded =
      Base64.getEncoder().encodeToString(cipher.iv + cipher.doFinal(value.toByteArray()))
...
/** Performs the one-time unbound registration and persists credentials only after verification. */
class DeviceBootstrapClient(
  private val baseUrl: String,
  private val identity: AndroidDeviceIdentity,
  private val secrets: DeviceCredentialStore
) {
  ...
  fun begin(
    ...
    val challenge =
      call(
        ...
        JSONObject().put("device_instance_id", identity.deviceInstanceId)
      )
    ...
    val digest =
      MessageDigest.getInstance("SHA-256").digest(capabilities.toByteArray()).joinToString("") {
        "%02x".format(it)
      }
    val proof =
      listOf(
    ...
    val request =
      JSONObject()
        ...
        .put("challenge", nonce)
        .put("device_instance_id", identity.deviceInstanceId)
        .put("device_public_key_jwk", jwk)
        .put("vendor", vendor.uppercase())
        .put("model", model)
        .put("serial", serial)
        .put("fmin", fmin)
        .put("firmware", firmware)
        .put("capability_digest", digest)
        .put("requested_roles", JSONArray(roles))
        .put("signature", identity.sign(proof))
    val created = call("/device-bootstrap/v1/activation-requests", request)
    secrets.put("pending", created.toString())
    return PendingActivation(
      created.getString("activation_request_id"),
      created.getString("request_secret"),
      created.getString("user_code"),
      created.getString("verification_uri"),
      created.getString("expires_at")
    )
  }

  fun pollCredential(): ActivatedBinding {
    val pending = JSONObject(secrets.get("pending") ?: error("NO_PENDING_ACTIVATION"))
    val id = pending.getString("activation_request_id")
    val secret = pending.getString("request_secret")
    val nonce = UUID.randomUUID().toString()
    val secretHash =
      MessageDigest.getInstance("SHA-256").digest(secret.toByteArray()).joinToString("") {
        "%02x".format(it)
      }
    val result =
      call(
        "/device-bootstrap/v1/activation-requests/$id/credential",
        JSONObject()
          .put("request_secret", secret)
          .put("nonce", nonce)
          .put("signature", identity.sign("credential\n$id\n$nonce\n$secretHash"))
      )
    val roles =
      result.getJSONArray("roles").let { values ->
        (0 until values.length()).map { values.getString(it) }
      }
    val binding =
      ActivatedBinding(
        result.getString("credential_id"),
        result.getString("client_certificate_pem"),
        result.getString("ca_chain_pem"),
        result.getString("mqtt_tls_uri"),
        result.optString("mqtt_wss_uri").ifBlank { null },
        result.getString("device_instance_id"),
        result.getString("organization_id"),
        result.getString("location_id"),
        result.getString("register_id"),
        roles,
        result.getLong("binding_version"),
        result.getString("command_hmac_key"),
        result.getString("sync_ack_hmac_key"),
        result.getString("ble_ticket_hmac_key"),
        result.getString("unp_prefix"),
        result.getLong("unp_range_start"),
        result.getLong("unp_range_end"),
        result.optString("local_token_issuer").ifBlank { null },
        result.optString("local_token_signing_kid").ifBlank { null },
        result.optString("local_token_public_key_der_base64").ifBlank { null },
    ...
    require(
      DeviceTLS.verifyCA(
        binding,
        CanonicalJson.encode(signedBinding).toByteArray(),
        result.getString("binding_signature")
      )
    ) {
      "BINDING_SIGNATURE_INVALID"
    }
    secrets.put("binding", result.toString())
    return binding
  }
}
