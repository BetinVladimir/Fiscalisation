...
test("collectCursorPages returns every page and preserves filters", async () => {
  const paths: string[] = [];
  const result = await collectCursorPages<number>(
  ...
  assert.deepEqual(paths, [
    "/orders?state=COMPLETED&limit=100",
    "/orders?state=COMPLETED&limit=100&cursor=two%20words",
  ]);
});

test("collectCursorPages rejects repeated cursors", async () => {
  await assert.rejects(
    collectCursorPages("/products", async () => ({
      items: [],
      page: { has_more: true, next_cursor: "repeat" },
    })),
    /INVALID_PAGINATION_SEQUENCE/,
  );
});
