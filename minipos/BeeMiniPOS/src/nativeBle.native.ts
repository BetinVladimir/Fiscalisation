...
import {base64urlDecode,base64urlEncode,randomBytes} from "./portableCrypto.ts";
...
class NativeBleBootstrap implements NativeBleBootstrapContract{
 private manager=new BleManager();private deviceId?:string;private session?:BleSessionPackage;private ready=false;private results=new BleResultWaiters();private frames=new FrameAssembler();
 get canSendComplianceIntent(){return this.ready}
 prepareSessionPublicKey(){return base64urlEncode(randomBytes(32))}
 resetSession(){this.results.cancelAll();if(this.deviceId)void this.manager.cancelDeviceConnection(this.deviceId).catch(()=>{});this.deviceId=undefined;this.session=undefined;this.ready=false;this.results=new BleResultWaiters()}
 async connectFromUserGesture(session:BleSessionPackage,onEvent:(raw:Uint8Array)=>void){if(session.security_mode!=="OPEN_MVP")throw new Error("BLE_SECURITY_MODE_UNSUPPORTED");if(new Date(session.expires_at).getTime()<=Date.now())throw new Error("BLE_ROUTE_PACKAGE_EXPIRED");const advertisingIdentity=authorizedAdvertisingIdentity(session.advertising_identity,session.edge_id);const device=await new Promise<any>((resolve,reject)=>{let done=false;const timer=setTimeout(()=>{if(done)return;done=true;this.manager.stopDeviceScan();reject(new Error("BLE_SCAN_TIMEOUT"))},10_000);this.manager.startDeviceScan([session.service_uuid],null,(error,found)=>{if(done)return;if(error){done=true;clearTimeout(timer);this.manager.stopDeviceScan();reject(error);return}if(found&&matchesAdvertisingIdentity(found,advertisingIdentity)){done=true;clearTimeout(timer);this.manager.stopDeviceScan();resolve(found)}})});const connected=await device.connect();await connected.discoverAllServicesAndCharacteristics();if(Platform.OS==="android")await connected.requestMTU(185);this.deviceId=connected.id;this.session=session;
  connected.monitorCharacteristicForService(session.service_uuid,session.event_characteristic_uuid,(error:BleError|null,characteristic:Characteristic|null)=>{if(error){this.ready=false;this.results.cancelAll();return}if(!characteristic?.value)return;void this.frames.accept(fromStandardBase64(characteristic.value)).then(done=>{if(!done)return;this.results.accept(done.id,done.payload);onEvent(done.payload)}).catch(()=>{})});this.ready=true;return{state:"OPEN_READY" as const,ready:Promise.resolve()}
 }
 async sendComplianceIntentAndWait(intent:unknown,messageId:string,_attMtu=185,timeoutMs=30_000){if(!this.ready||!this.session)throw new Error("BLE_NOT_READY");const result=this.results.wait(messageId,timeoutMs);try{for(const frame of await frameMessage(encodeCanonical(intent),messageId,_attMtu))await this.write(this.session.command_characteristic_uuid,frame)}catch(e){this.results.reject(messageId,e)}return result}
