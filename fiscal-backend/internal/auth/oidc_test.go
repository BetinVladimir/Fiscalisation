...
func TestOIDCVerifierValidatesAndRefreshesRotatedKid(t *testing.T) {
	...
	defer server.Close()
	verifier := NewOIDCVerifier("https://issuer.example/", "beefiscal", server.URL)
	if claims, err := verifier.Parse(oidcToken(t, first, "first", "https://issuer.example", []string{"other", "beefiscal"}), time.Now()); err != nil || claims.TenantID != "tenant-1" || claims.Issuer != "https://issuer.example" {
		t.Fatalf("valid token rejected: %#v %v", claims, err)
	}
	keys.set("second", second)
	if _, err := verifier.Parse(oidcToken(t, second, "second", "https://issuer.example", "beefiscal"), time.Now().Add(6*time.Second)); err != nil {
...
func TestOIDCVerifierRejectsWrongTrustClaimsAndSignature(t *testing.T) {
	...
	defer server.Close()
	verifier := NewOIDCVerifier("https://issuer.example", "beefiscal", server.URL)
	for name, token := range map[string]string{"issuer": oidcToken(t, key, "trusted", "https://evil.example", "beefiscal"), "audience": oidcToken(t, key, "trusted", "https://issuer.example", "other"), "signature": oidcToken(t, attacker, "trusted", "https://issuer.example", "beefiscal")} {
