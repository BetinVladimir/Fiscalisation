...
func (p *Postgres) BeginOutboundEmail(ctx context.Context, recipient, subject, messageText string) (string, error) {
	var id string
	err := p.db.QueryRowContext(ctx, `insert into fiscal.outbound_email_messages(recipient,subject,message_text,status) values($1,$2,$3,'SENDING') returning id::text`, recipient, subject, messageText).Scan(&id)
	return id, err
}

func (p *Postgres) MarkOutboundEmailSent(ctx context.Context, id string) error {
	result, err := p.db.ExecContext(ctx, `update fiscal.outbound_email_messages set status='SENT',sent_at=now(),updated_at=now(),last_error=null where id=$1 and status='SENDING'`, id)
...
func (p *Postgres) MarkOutboundEmailFailed(ctx context.Context, id, failure string) error {
	result, err := p.db.ExecContext(ctx, `update fiscal.outbound_email_messages set status='FAILED',updated_at=now(),last_error=$2 where id=$1 and status='SENDING'`, id, failure)
	return ensureOutboundEmailUpdated(result, err)
}
...
func (p *Postgres) AllocateUNP(ctx context.Context, tenantID, allocationID, fiscalDeviceNumber, operatorCode string) (string, int64, error) {
	...
	var sequence int64
	if err = tx.QueryRowContext(ctx, `select coalesce(max(unp_sequence),0)+1 from fiscal.unp_allocations where tenant_id=$1 and fiscal_device_number=$2`, tenantID, fiscalDeviceNumber).Scan(&sequence); err != nil {
		return "", 0, err
	}
	if sequence > 9999999 {
	...
	if _, err = tx.ExecContext(ctx, `insert into fiscal.unp_allocations(tenant_id,allocation_id,fiscal_device_number,operator_code,unp_sequence,unp,status) values($1,$2,$3,$4,$5,$6,'ALLOCATED')`, tenantID, allocationID, fiscalDeviceNumber, operatorCode, sequence, unp); err != nil {
		return "", 0, err
	}
...
func Open(url string) (*Postgres, error) {
	...
	if _, e = db.ExecContext(ctx, `alter table fiscal.fiscal_state_meta add column if not exists storage_mode smallint not null default 1`); e != nil {
		db.Close()
		return nil, e
	}
...
func (p *Postgres) Load() ([]byte, error) {
	rows, err := p.db.Query(`select collection,entity_key,payload from fiscal.fiscal_state_rows order by collection,entity_key`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	metaErr := p.db.QueryRow(`select generation from fiscal.fiscal_state_meta where singleton=true`).Scan(&generation)
	...
	var legacy []byte
	err = p.db.QueryRow(`select payload from fiscal.runtime_snapshots where aggregate='fiscal'`).Scan(&legacy)
...
func (p *Postgres) LoadVersioned() ([]byte, int64, error) {
	...
	var generation int64
	var storageMode int
	metaErr := tx.QueryRow(`select generation,storage_mode from fiscal.fiscal_state_meta where singleton=true`).Scan(&generation, &storageMode)
	if metaErr != nil && !errors.Is(metaErr, sql.ErrNoRows) {
	...
	rows, err := tx.Query(`select collection,entity_key,payload from fiscal.fiscal_state_rows order by collection,entity_key`)
	if err != nil {
		return nil, 0, err
	}
	flat := make([]stateRow, 0)
...
func (p *Postgres) SaveDeltaVersioned(previous, current []byte, expected int64) (int64, error) {
	...
	var actual int64
	storageMode := 1
	err = tx.QueryRow(`select generation,storage_mode from fiscal.fiscal_state_meta where singleton=true for update`).Scan(&actual, &storageMode)
	if errors.Is(err, sql.ErrNoRows) {
		actual, err = 0, nil
	}
	if err != nil {
	...
	for _, row := range deletes {
		if storageMode == 1 {
			if _, err = tx.Exec(`delete from fiscal.fiscal_state_rows where collection=$1 and entity_key=$2`, row.Collection, row.Key); err != nil {
				return 0, err
			}
		}
...
select collection,entity_key,payload from (
  select 'sales'::text collection,id entity_key,payload,'0'::text ordering from fiscal.fiscal_runtime_sales
  union all select 'operations',id,payload,'0' from fiscal.fiscal_runtime_operations
  union all select 'shifts',id,payload,'0' from fiscal.fiscal_runtime_shifts
  union all select 'resources',kind||':'||id,payload,'0' from fiscal.fiscal_runtime_resources where kind<>'composite_binding'
  union all select 'resources','composite_binding:'||binding_id,payload,'0' from fiscal.fiscal_runtime_composite_bindings
  union all select 'outbox',id,payload,'0' from fiscal.fiscal_runtime_outbox
  union all select 'ble_sessions',id,payload,'0' from fiscal.fiscal_runtime_ble_sessions
  union all select 'connectivity_probes',id,payload,'0' from fiscal.fiscal_runtime_connectivity_probes
  union all select 'edge_pending',operation_id,payload,'0' from fiscal.fiscal_runtime_edge_pending
  union all select 'audit',event_id,payload,occurred_at::text from fiscal.fiscal_runtime_audit
  union all select 'replays',replay_key,payload,'0' from fiscal.fiscal_runtime_replays
  union all select 'unp',tenant_id||E'\n'||register_id,to_jsonb(last_sequence),'0' from fiscal.fiscal_runtime_unp_sequences
  union all select 'artifacts',tenant_id||E'\n'||id,to_jsonb(encode(body,'base64')),'0' from fiscal.fiscal_runtime_artifacts
  union all select 'sync_acks',tenant_id||E'\n'||edge_id,payload,'0' from fiscal.fiscal_runtime_sync_acks
  union all select 'activation_challenges',activation_challenge_id::text,payload,'0' from fiscal.fiscal_device_activation_challenges
  union all select 'activation_requests',activation_request_id::text,payload,'0' from fiscal.fiscal_device_activation_requests
...
func fiscalTypedCoverageComplete(tx *sql.Tx) (bool, error) {
	var missing int
	err := tx.QueryRow(`select count(*) from fiscal.fiscal_state_rows s where s.collection <> 'devices' and not (
  (s.collection='sales' and exists(select 1 from fiscal.fiscal_runtime_sales t where t.id=s.entity_key and t.payload=s.payload)) or
  (s.collection='operations' and exists(select 1 from fiscal.fiscal_runtime_operations t where t.id=s.entity_key and t.payload=s.payload)) or
  (s.collection='shifts' and exists(select 1 from fiscal.fiscal_runtime_shifts t where t.id=s.entity_key and t.payload=s.payload)) or
  (s.collection='resources' and exists(select 1 from fiscal.fiscal_runtime_resources t where t.kind||':'||t.id=s.entity_key and t.payload=s.payload)) or
  (s.collection='outbox' and exists(select 1 from fiscal.fiscal_runtime_outbox t where t.id=s.entity_key and t.payload=s.payload)) or
  (s.collection='ble_sessions' and exists(select 1 from fiscal.fiscal_runtime_ble_sessions t where t.id=s.entity_key and t.payload=s.payload)) or
  (s.collection='connectivity_probes' and exists(select 1 from fiscal.fiscal_runtime_connectivity_probes t where t.id=s.entity_key and t.payload=s.payload)) or
  (s.collection='edge_pending' and exists(select 1 from fiscal.fiscal_runtime_edge_pending t where t.operation_id=s.entity_key and t.payload=s.payload)) or
  (s.collection='audit' and exists(select 1 from fiscal.fiscal_runtime_audit t where t.payload=s.payload)) or
  (s.collection='replays' and exists(select 1 from fiscal.fiscal_runtime_replays t where t.replay_key=s.entity_key and t.payload=s.payload)) or
  (s.collection='unp' and exists(select 1 from fiscal.fiscal_runtime_unp_sequences t where t.tenant_id||E'\n'||t.register_id=s.entity_key and to_jsonb(t.last_sequence)=s.payload)) or
  (s.collection='artifacts' and exists(select 1 from fiscal.fiscal_runtime_artifacts t where t.tenant_id||E'\n'||t.id=s.entity_key and to_jsonb(encode(t.body,'base64'))=s.payload)) or
  (s.collection='sync_acks' and exists(select 1 from fiscal.fiscal_runtime_sync_acks t where t.tenant_id||E'\n'||t.edge_id=s.entity_key and t.payload=s.payload))
  or (s.collection='activation_challenges' and exists(select 1 from fiscal.fiscal_device_activation_challenges t where t.activation_challenge_id::text=s.entity_key and t.payload=s.payload))
  or (s.collection='activation_requests' and exists(select 1 from fiscal.fiscal_device_activation_requests t where t.activation_request_id::text=s.entity_key and t.payload=s.payload))
)`).Scan(&missing)
...
func (p *Postgres) saveVersioned(b []byte, expected *int64) (int64, error) {
	...
	defer tx.Rollback()
	var current int64
	err = tx.QueryRow(`select generation from fiscal.fiscal_state_meta where singleton=true for update`).Scan(&current)
	if errors.Is(err, sql.ErrNoRows) {
		current, err = 0, nil
	}
	if err != nil {
	...
	existing, err := tx.Query(`select collection,entity_key,payload from fiscal.fiscal_state_rows for update`)
	if err != nil {
		return 0, err
	}
	...
	stmt, err := tx.Prepare(`insert into fiscal.fiscal_state_rows(collection,entity_key,payload,updated_at) values($1,$2,$3::jsonb,now()) on conflict(collection,entity_key) do update set payload=excluded.payload,updated_at=now() where fiscal_state_rows.payload is distinct from excluded.payload`)
	if err != nil {
		return 0, err
	}
	defer stmt.Close()
	...
	var generation int64
	if err = tx.QueryRow(`insert into fiscal.fiscal_state_meta(singleton,generation,updated_at) values(true,1,now()) on conflict(singleton) do update set generation=fiscal_state_meta.generation+1,updated_at=excluded.updated_at returning generation`).Scan(&generation); err != nil {
		return 0, err
	}
	if err = tx.Commit(); err != nil {
		return 0, err
...
func (p *Postgres) LoadTenantEntity(collection, tenant, id string) ([]byte, error) {
	...
	switch collection {
	case "sales":
		err = tx.QueryRow(`select jsonb_build_object('sale_id',id,'tenant_id',tenant_id,'external_id',external_id,'location_id',coalesce(location_id,''),'register_id',register_id,'operator_id',operator_id,'location_code',coalesce(payload->>'location_code',''),'location_name',coalesce(payload->>'location_name',''),'register_code',coalesce(payload->>'register_code',''),'operator_code',coalesce(payload->>'operator_code',''),'operator_name',coalesce(payload->>'operator_name',''),'unp',coalesce(unp,''),'state',state,'version',version,'lines',lines,'payments',payments,'fiscal_operation_id',coalesce(fiscal_operation_id,''),'receipt_artifact_id',coalesce(receipt_artifact_id,''),'fiscal_device',jsonb_strip_nulls(jsonb_build_object('device_id',fiscal_device_id,'binding_version',coalesce((payload->'fiscal_device'->>'binding_version')::bigint,0),'serial',fiscal_device_serial,'fiscal_device_number',fiscal_device_number,'fiscal_memory_number',fiscal_memory_number,'vendor',fiscal_device_vendor,'model',fiscal_device_model,'firmware',fiscal_device_firmware)),'created_at',created_at,'updated_at',updated_at,'completed_at',payload->'completed_at','cancelled_at',payload->'cancelled_at','reversed_at',payload->'reversed_at') from fiscal.fiscal_runtime_sales where id=$1`, id).Scan(&raw)
	case "operations":
		err = tx.QueryRow(`select payload from fiscal.fiscal_runtime_operations where id=$1`, id).Scan(&raw)
	case "shifts":
		err = tx.QueryRow(`select jsonb_build_object('id',id,'tenant_id',tenant_id,'register_id',register_id,'operator_id',operator_id,'state',state,'version',version,'opened_at',opened_at,'closed_at',closed_at) from fiscal.fiscal_runtime_shifts where id=$1`, id).Scan(&raw)
	case "ble_sessions":
		err = tx.QueryRow(`select payload from fiscal.fiscal_runtime_ble_sessions where id=$1`, id).Scan(&raw)
	case "connectivity_probes":
		err = tx.QueryRow(`select payload from fiscal.fiscal_runtime_connectivity_probes where id=$1`, id).Scan(&raw)
	case "edge_pending":
		err = tx.QueryRow(`select payload from fiscal.fiscal_runtime_edge_pending where operation_id=$1`, id).Scan(&raw)
	case "sync_acks":
		err = tx.QueryRow(`select payload from fiscal.fiscal_runtime_sync_acks where edge_id=$1`, id).Scan(&raw)
	case "artifacts":
		err = tx.QueryRow(`select to_json(encode(body,'base64')) from fiscal.fiscal_runtime_artifacts where id=$1`, id).Scan(&raw)
	case "replays":
		err = tx.QueryRow(`select payload from fiscal.fiscal_runtime_replays where replay_key=$1`, id).Scan(&raw)
	default:
		if len(collection) > len("resources:") && collection[:len("resources:")] == "resources:" {
			err = tx.QueryRow(`select jsonb_build_object('kind',kind,'tenant_id',tenant_id,'id',id,'version',version,'data',data,'created_at',created_at,'updated_at',updated_at) from fiscal.fiscal_runtime_resources where kind=$1 and id=$2`, collection[len("resources:"):], id).Scan(&raw)
		} else {
...
func (p *Postgres) LoadSystemEntities(collection string) ([][]byte, error) {
	...
	switch collection {
	case "outbox":
		query = `select payload from fiscal.fiscal_runtime_outbox order by next_attempt,id`
	default:
...
func (p *Postgres) LoadTenantEntities(collection, tenant string) ([][]byte, error) {
	...
	switch collection {
	case "sales":
		query = `select jsonb_build_object('sale_id',id,'tenant_id',tenant_id,'external_id',external_id,'location_id',coalesce(location_id,''),'register_id',register_id,'operator_id',operator_id,'location_code',coalesce(payload->>'location_code',''),'location_name',coalesce(payload->>'location_name',''),'register_code',coalesce(payload->>'register_code',''),'operator_code',coalesce(payload->>'operator_code',''),'operator_name',coalesce(payload->>'operator_name',''),'unp',coalesce(unp,''),'state',state,'version',version,'lines',lines,'payments',payments,'fiscal_operation_id',coalesce(fiscal_operation_id,''),'receipt_artifact_id',coalesce(receipt_artifact_id,''),'fiscal_device',jsonb_strip_nulls(jsonb_build_object('device_id',fiscal_device_id,'binding_version',coalesce((payload->'fiscal_device'->>'binding_version')::bigint,0),'serial',fiscal_device_serial,'fiscal_device_number',fiscal_device_number,'fiscal_memory_number',fiscal_memory_number,'vendor',fiscal_device_vendor,'model',fiscal_device_model,'firmware',fiscal_device_firmware)),'created_at',created_at,'updated_at',updated_at,'completed_at',payload->'completed_at','cancelled_at',payload->'cancelled_at','reversed_at',payload->'reversed_at') from fiscal.fiscal_runtime_sales order by created_at,id`
	case "operations":
		query = `select payload from fiscal.fiscal_runtime_operations order by created_at,id`
	case "shifts":
		query = `select jsonb_build_object('id',id,'tenant_id',tenant_id,'register_id',register_id,'operator_id',operator_id,'state',state,'version',version,'opened_at',opened_at,'closed_at',closed_at) from fiscal.fiscal_runtime_shifts order by opened_at,id`
	case "ble_sessions":
		query = `select payload from fiscal.fiscal_runtime_ble_sessions order by expires_at,id`
	case "connectivity_probes":
		query = `select payload from fiscal.fiscal_runtime_connectivity_probes order by observed_at,id`
	case "edge_pending":
		query = `select payload from fiscal.fiscal_runtime_edge_pending order by accepted_at,operation_id`
	case "sync_acks":
		query = `select payload from fiscal.fiscal_runtime_sync_acks order by committed_at,edge_id`
	case "audit":
		query = `select payload from fiscal.fiscal_runtime_audit order by occurred_at,event_id`
	default:
		if len(collection) > len("resources:") && collection[:len("resources:")] == "resources:" {
			query = `select jsonb_build_object('kind',kind,'tenant_id',tenant_id,'id',id,'version',version,'data',data,'created_at',created_at,'updated_at',updated_at) from fiscal.fiscal_runtime_resources where kind=$1 order by updated_at,id`
...
func deleteTypedProjection(tx *sql.Tx, row stateRow) error {
	if row.Collection == "activation_challenges" {
		_, err := tx.Exec(`delete from fiscal.fiscal_device_activation_challenges where activation_challenge_id=$1`, row.Key)
		return err
	}
	if row.Collection == "activation_requests" {
		_, err := tx.Exec(`delete from fiscal.fiscal_device_activation_requests where activation_request_id=$1`, row.Key)
		return err
	}
	if row.Collection == "sync_acks" {
		...
		return withTenantRole(tx, row, func() error {
			_, err := tx.Exec(`delete from fiscal.fiscal_runtime_sync_acks where tenant_id=$1 and edge_id=$2`, parts[0], parts[1])
			return err
		})
	}
	if row.Collection == "artifacts" {
		...
		return withTenantRole(tx, row, func() error {
			_, err := tx.Exec(`delete from fiscal.fiscal_runtime_artifacts where tenant_id=$1 and id=$2`, parts[0], parts[1])
			return err
		})
	}
	if row.Collection == "unp" {
		...
		return withTenantRole(tx, row, func() error {
			_, err := tx.Exec(`delete from fiscal.fiscal_runtime_unp_sequences where tenant_id=$1 and register_id=$2`, parts[0], parts[1])
			return err
		})
	}
	if row.Collection == "resources" {
		return withTenantRole(tx, row, func() error {
			...
			if _, err := tx.Exec(`delete from fiscal.fiscal_runtime_resources where kind=$1 and id=$2`, identity.Kind, identity.ID); err != nil {
				return err
			}
			if identity.Kind == "composite_binding" {
				_, err := tx.Exec(`delete from fiscal.fiscal_runtime_composite_bindings where binding_id=$1`, identity.ID)
				return err
			}
			return nil
		})
...
func upsertTypedProjectionAsTenant(tx *sql.Tx, row stateRow) error {
	...
	switch row.Collection {
	case "sales":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_sales(id,tenant_id,external_id,location_id,register_id,operator_id,unp,state,version,lines,payments,fiscal_operation_id,receipt_artifact_id,fiscal_device_id,fiscal_device_serial,fiscal_device_number,fiscal_memory_number,fiscal_device_vendor,fiscal_device_model,fiscal_device_firmware,created_at,updated_at,payload)
...
where fiscal_runtime_sales is distinct from excluded`, row.Key, payload)
		return err
	case "operations":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_operations(id,tenant_id,sale_id,register_id,type,state,version,fiscal_reference,original_fiscal_reference,reason_code,simulated,error_code,created_at,updated_at,payload)
...
where fiscal_runtime_operations is distinct from excluded`, row.Key, payload)
		return err
	case "shifts":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_shifts(id,tenant_id,register_id,operator_id,state,version,opened_at,closed_at,payload)
...
where fiscal_runtime_shifts is distinct from excluded`, row.Key, payload)
		return err
	case "resources":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_resources(kind,id,tenant_id,version,data,created_at,updated_at,payload)
...
where fiscal_runtime_resources is distinct from excluded`, payload)
		...
		_, err = tx.Exec(`insert into fiscal.fiscal_runtime_composite_bindings(binding_id,tenant_id,register_id,profile,adapter_device_id,fiscal_device_id,payment_device_id,generation,applied_generation,status,concurrency_version,payload,updated_at)
...
on conflict(binding_id)do update set tenant_id=excluded.tenant_id,register_id=excluded.register_id,profile=excluded.profile,adapter_device_id=excluded.adapter_device_id,fiscal_device_id=excluded.fiscal_device_id,payment_device_id=excluded.payment_device_id,generation=excluded.generation,applied_generation=excluded.applied_generation,status=excluded.status,concurrency_version=excluded.concurrency_version,payload=excluded.payload,updated_at=excluded.updated_at where fiscal_runtime_composite_bindings is distinct from excluded`, payload)
		return err
	case "outbox":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_outbox(id,tenant_id,event_type,resource_id,attempts,next_attempt,delivered_at,payload)
...
on conflict(id) do update set tenant_id=excluded.tenant_id,event_type=excluded.event_type,resource_id=excluded.resource_id,attempts=excluded.attempts,next_attempt=excluded.next_attempt,delivered_at=excluded.delivered_at,payload=excluded.payload where fiscal_runtime_outbox is distinct from excluded`, row.Key, payload)
		return err
	case "ble_sessions":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_ble_sessions(id,tenant_id,location_id,register_id,operator_id,app_instance_id,actor_subject,client_public_key,device_id,fiscal_device_id,fencing_token,expires_at,revoked,payload)
...
on conflict(id) do update set tenant_id=excluded.tenant_id,location_id=excluded.location_id,register_id=excluded.register_id,operator_id=excluded.operator_id,app_instance_id=excluded.app_instance_id,actor_subject=excluded.actor_subject,client_public_key=excluded.client_public_key,device_id=excluded.device_id,fiscal_device_id=excluded.fiscal_device_id,fencing_token=excluded.fencing_token,expires_at=excluded.expires_at,revoked=excluded.revoked,payload=excluded.payload where fiscal_runtime_ble_sessions is distinct from excluded`, row.Key, payload)
		return err
	case "connectivity_probes":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_connectivity_probes(id,tenant_id,register_id,state,observed_at,recommended_transport,payload)
...
on conflict(id) do update set tenant_id=excluded.tenant_id,register_id=excluded.register_id,state=excluded.state,observed_at=excluded.observed_at,recommended_transport=excluded.recommended_transport,payload=excluded.payload where fiscal_runtime_connectivity_probes is distinct from excluded`, row.Key, payload)
		return err
	case "edge_pending":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_edge_pending(operation_id,tenant_id,register_id,device_id,command_type,operation_sequence,unp_sequence,accepted_at,payload)
...
on conflict(operation_id) do update set tenant_id=excluded.tenant_id,register_id=excluded.register_id,device_id=excluded.device_id,command_type=excluded.command_type,operation_sequence=excluded.operation_sequence,unp_sequence=excluded.unp_sequence,accepted_at=excluded.accepted_at,payload=excluded.payload where fiscal_runtime_edge_pending is distinct from excluded`, row.Key, payload)
		return err
	case "audit":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_audit(event_id,tenant_id,actor_id,action,object_type,object_id,occurred_at,event_hash,payload)
...
on conflict(event_id) do nothing`, payload)
		return err
	case "replays":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_replays(replay_key,tenant_id,method,path,request_hash,status,pending,payload)
...
on conflict(replay_key) do update set tenant_id=excluded.tenant_id,method=excluded.method,path=excluded.path,request_hash=excluded.request_hash,status=excluded.status,pending=excluded.pending,payload=excluded.payload where fiscal_runtime_replays is distinct from excluded`, row.Key, payload)
		return err
	case "unp":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_unp_sequences(tenant_id,register_id,last_sequence)
...
on conflict(tenant_id,register_id) do update set last_sequence=excluded.last_sequence where fiscal_runtime_unp_sequences.last_sequence is distinct from excluded.last_sequence`, row.Key, payload)
		return err
	case "artifacts":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_artifacts(tenant_id,id,body,sha256)
values(split_part($1,E'\n',1),split_part($1,E'\n',2),decode($2::jsonb#>>'{}','base64'),encode(fiscal.digest(decode($2::jsonb#>>'{}','base64'),'sha256'),'hex'))
...
where fiscal_runtime_artifacts is distinct from excluded`, row.Key, payload)
		return err
	case "sync_acks":
		_, err := tx.Exec(`insert into fiscal.fiscal_runtime_sync_acks(tenant_id,edge_id,ack_id,committed_through_seq,committed_event_hash,committed_at,payload)
