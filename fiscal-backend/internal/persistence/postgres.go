...
func upsertTypedProjectionAsTenant(tx *sql.Tx, row stateRow) error {
	...
	switch row.Collection {
	case "sales":
		_, err := tx.Exec(`insert into fiscal_runtime_sales(id,tenant_id,external_id,register_id,operator_id,unp,state,version,lines,payments,fiscal_operation_id,receipt_artifact_id,created_at,updated_at,payload)
...
where fiscal_runtime_sales is distinct from excluded`, row.Key, payload)
		return err
	case "operations":
		_, err := tx.Exec(`insert into fiscal_runtime_operations(id,tenant_id,sale_id,type,state,version,fiscal_reference,original_fiscal_reference,reason_code,simulated,error_code,created_at,updated_at,payload)
values($1,$2::jsonb->>'tenant_id',nullif($2::jsonb->>'sale_id',''),$2::jsonb->>'type',$2::jsonb->>'state',($2::jsonb->>'version')::bigint,nullif($2::jsonb->>'fiscal_reference',''),nullif($2::jsonb->>'original_fiscal_reference',''),nullif($2::jsonb->>'reason_code',''),coalesce(($2::jsonb->>'simulated')::boolean,false),nullif($2::jsonb->>'error_code',''),($2::jsonb->>'created_at')::timestamptz,($2::jsonb->>'updated_at')::timestamptz,$2::jsonb)
on conflict(id) do update set tenant_id=excluded.tenant_id,sale_id=excluded.sale_id,type=excluded.type,state=excluded.state,version=excluded.version,fiscal_reference=excluded.fiscal_reference,original_fiscal_reference=excluded.original_fiscal_reference,reason_code=excluded.reason_code,simulated=excluded.simulated,error_code=excluded.error_code,created_at=excluded.created_at,updated_at=excluded.updated_at,payload=excluded.payload
...
on conflict(event_id) do update set tenant_id=excluded.tenant_id,actor_id=excluded.actor_id,action=excluded.action,object_type=excluded.object_type,object_id=excluded.object_id,occurred_at=excluded.occurred_at,event_hash=excluded.event_hash,payload=excluded.payload where fiscal_runtime_audit is distinct from excluded`, payload)
		return err
	case "replays":
		_, err := tx.Exec(`insert into fiscal_runtime_replays(replay_key,tenant_id,method,path,request_hash,status,payload)
values($1,split_part($1,' ',1),split_part($1,' ',2),split_part($1,' ',3),$2::jsonb->>'hash',($2::jsonb->>'status')::integer,$2::jsonb)
on conflict(replay_key) do update set tenant_id=excluded.tenant_id,method=excluded.method,path=excluded.path,request_hash=excluded.request_hash,status=excluded.status,payload=excluded.payload where fiscal_runtime_replays is distinct from excluded`, row.Key, payload)
		return err
	case "unp":
