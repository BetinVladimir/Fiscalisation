...
func TestThreeProfilesContinueOneOperationAcrossRESTBLEAndMQTT(t *testing.T) {
	for _, profile := range []string{"DATECS_BLUECASH50_EMBEDDED", "DATECS_DP150_BLUEPAD50", "DAISY_COMPACT_S01"} {
		t.Run(profile, func(t *testing.T) {
			...
			repo, _ = NewPersistentRepository(store); svc = NewService(repo, NewSimulator(true)); svc.SetBLESigningKey(string(key))
			if replay, e := svc.SyncBatchForTenant("tenant-1", signCustomBatch("edge-1", 1, nil, []DeviceEventEnvelope{accepted}, key)); e != nil || replay.CommittedThroughSeq != ack1.CommittedThroughSeq { t.Fatal(replay, e) }
			...
			if err != nil || ack2.CommittedThroughSeq != 2 { t.Fatal(ack2, err) }
			op, err := svc.GetOperation(opID); if err != nil || op.State != "FISCALIZED" || op.ID != opID { t.Fatal(op, err) }
			if pending := svc.PendingOutbox(time.Now().UTC()); len(pending) != 1 { t.Fatalf("webhook was not deduplicated: %#v", pending) }
		})
	}
}
